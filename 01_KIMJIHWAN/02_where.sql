...
-- AND &&    여러 조건이 동시에 TRUE일 경우에만 TRUE 값 반환    // 앞이 False이면
-- OR     여러 조건들 중에 어느 하나의 조건만 TRUE이면 TRUE값 반환  // 앞이 True이면
...
-- 메뉴테이블에서 메뉴가격이 5000원 보다 크고, 카테고리 코드가 10인 메뉴를 출력해주세요
...
-- 우선순위 (SQL)
-- 나열한 AND, OR 중에는 AND 연산이 우선순위가 높다.
SELECT 1 OR 0 AND 0;
SELECT (1 OR 0) AND 0;
...
-- 2. _개수에 따라 문자 1개를 의미한다 _가 3개라면 문자 3개를 의미한다. -빈칸
...
#       IFNULL(ref_category_code, 0) = 0;
    COALESCE(ref_category_code, 0) = 0;  -- 모든 dbms에서 사용가능
